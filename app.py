...
# --- CONFIGURAÇÕES DE SEGURANÇA E ARQUIVO ---
NOME_ARQUIVO = "STATUS DOS INVENTÁRIOS.xlsm"

# 1. Senha Mestra do Administrador
SENHA_ADMIN = "admin123"

# 2. Mapeamento de Todos os Centros -> Token Secreto
TOKENS_CENTROS = {
    "B001": "sec-b001-848d51",
    "B002": "sec-b002-ee6209",
    "B006": "sec-b006-e47c36",
    "B007": "sec-b007-d70169",
    "B008": "sec-b008-0cd60e",
    "B009": "sec-b009-ae992a",
    "B010": "sec-b010-aa4ef8",
    "B011": "sec-b011-92d444",
    "B012": "sec-b012-ad7041",
    "B013": "sec-b013-33d2a2",
    "B015": "sec-b015-997afc",
    "B016": "sec-b016-7b8dbc",
    "B017": "sec-b017-19cf12",
    "B018": "sec-b018-fa65f7",
    "B019": "sec-b019-c2c222",
    "B020": "sec-b020-4a2aee",
    "B021": "sec-b021-61d218",
    "B022": "sec-b022-1a0541",
    "B023": "sec-b023-5439f1",
    "B024": "sec-b024-d55323",
    "B025": "sec-b025-267a2b",
    "B026": "sec-b026-e4415c",
    "B027": "sec-b027-954fb1",
    "B028": "sec-b028-e8d3cd",
    "B029": "sec-b029-3e6365",
    "B030": "sec-b030-66271f",
    "B031": "sec-b031-30485d"
}
...
if os.path.exists(NOME_ARQUIVO):
    try:
        df = load_data(NOME_ARQUIVO)

        st.sidebar.title("🔐 Controle de Acesso")

        query_params = st.query_params
        centro_url = query_params.get("centro", None)
        ...
        # --- REGRA DE ACESSO ---
        if centro_url:
            token_esperado = TOKENS_CENTROS.get(str(centro_url).upper())
            
            # Validação do Token do Link enviado para a Loja
            if token_url and token_esperado and token_url == token_esperado:
                lojas_sel = [centro_url.upper()]
                st.sidebar.success(f"🔒 **Acesso Restrito**\n\nCentro: **{centro_url.upper()}**")
            else:
                st.error("⛔ **Acesso Negado**: Link de acesso inválido ou sem permissão para esta loja.")
                st.info("Entre em contato com a administração para receber o link correto.")
                st.stop()
        else:
            # Entrada para Administrador (Exige Senha Mestra)
            senha_digitada = st.sidebar.text_input("Digite a Senha Mestra (Admin):", type="password")
            ...
            if senha_digitada == SENHA_ADMIN:
                ...
                st.info("👈 Por favor, insira a Senha Mestra na barra lateral para carregar a visão completa.")
                st.stop()
        ...
        if centro_url:
            st.caption(f"Exibindo dados exclusivos do centro: **{centro_url.upper()}**")
        ...
        # --- LINHA 1 DE GRÁFICOS (POR CENTRO) ---
        ...
        # --- TABELA TOP CENTROS COM MAIOR PERDA ---
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        df_top10_centros.rename(columns={
            'Loja_Nome': 'Centro',
            'Qtd_Limpa': 'Perda (Qtd)',
            'Valor_Limpo': 'Perda (R$)'
        }, inplace=True)
        ...
        # --- TABELA TOP MARCAS COM MAIOR PERDA ---
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        df_top10_marcas.rename(columns={
            'Marca_Nome': 'Marca',
            'Qtd_Limpa': 'Perda (Qtd)',
            'Valor_Limpo': 'Perda (R$)'
        }, inplace=True)
        ...
        # --- LINHA 3 DE GRÁFICOS (TODAS AS MARCAS) ---
        st.markdown("<br>", unsafe_allow_html=True)
        
        marca_col1, marca_col2 = st.columns(2)

        df_perdas_marcas = df_filtered[
            (df_filtered['Valor_Limpo'] < 0) | (df_filtered['Qtd_Limpa'] < 0)
        ]
        ...
        st.error(f"Erro ao processar o arquivo de inventário: {e}")
else:
    st.warning(f"O arquivo `{NOME_ARQUIVO}` não foi encontrado na mesma pasta do script Python.")