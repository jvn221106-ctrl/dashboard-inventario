...
# Funções auxiliares de formatação de valores para os KPIs
def formatar_moeda(val):
    """Formata valor em R$: mostra '-' apenas se val < 0, sem sinal para positivo."""
    if val < 0:
...
def formatar_qtd(val):
    """Formata quantidade: mostra '-' apenas se val < 0, sem sinal para positivo."""
    if val < 0:
...
try:
    ...
    # --- CÁLCULOS DOS KPIS ---
    # Soma dos valores negativos (perdas reais)
    ...
    # Soma dos valores positivos (sobras)
    ...
    # Exibição dos KPIs utilizando a nova formatação
    ...
    # Formatação da Tabela de Centros
    ...
    # Formatação da Tabela de Marcas
    ...
    # --- LINHA 3 DE GRÁFICOS (TODAS AS MARCAS - QTD À ESQUERDA, R$ À DIREITA) ---
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    # 1. ESQUERDA: Perda por Marca (Qtd) - Todas em Linha
    with marca_col1:
    ...
    # 2. DIREITA: Perda por Marca (R$) - Todas em Linha
    with marca_col2:
