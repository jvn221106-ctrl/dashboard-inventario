...
def renderizar_aba_admin():
    ...
    with col1:
        usuario_selecionado = st.selectbox("Selecione o e-mail:", options=list(usuarios.keys()))
        nova_senha = st.text_input("Nova Senha Temporária:", type="password", key="input_nova_senha")
    ...
    with col2:
        ...
        if st.button("Redefinir Senha", type="primary"):
            if not nova_senha or len(nova_senha) < 6:
                ...
                salvar_usuarios(usuarios)
                st.success(f"✅ Senha do e-mail **{usuario_selecionado}** atualizada!")
...
def renderizar_dashboard():
    ...
    try:
        ...
        if loja_usuario == "TODAS":
            ...
            # Se for gerente, restringe aos dados apenas da sua loja vinculada
        ...
        # --- LINHA 3 DE GRÁFICOS (TODAS AS MARCAS - LINHAS) ---
        st.markdown("<br>", unsafe_allow_html=True)
...
if not st.session_state["logado"]:
    ...
    # Barra Lateral com Informações de Logout
