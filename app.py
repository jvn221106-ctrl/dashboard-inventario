...
def load_data(uploaded_file):
    # Lê as primeiras 25 linhas sem cabeçalho para localizar a tabela
    ...
    for idx, row in df_preview.iterrows():
        ...
        if any(term in " ".join(row_str) for term in ['centro', 'loja', 'quantidade', 'fornecedor', 'marca', 'valor', 'qtd']):
            header_idx = idx
            break

    # Recarrega a planilha usando a linha do cabeçalho correta
    df = pd.read_excel(uploaded_file, engine='openpyxl', header=header_idx)
    df = df.dropna(how='all')
    
    # Limpa nomes de colunas (remove 'Unnamed', espaços extras e quebras de linha)
    novas_colunas = []
