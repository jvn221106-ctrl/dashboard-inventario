...
# LINK PARA O ARQUIVO DE DIVISÃO REGIONAL (Insira o link de download direto do SharePoint/OneDrive aqui se for um arquivo separado, ou ajuste se for uma aba do mesmo arquivo)
...
def load_data():
    ...
    # Tentativa de carregar a aba de inventário e se houver a aba regional na mesma pasta
    ...
    # 2. Carregar Planilha de Divisão Regional
    try:
        if "PADRÃO_DIVISÃO REGIONAL" in xls.sheet_names:
            ...
            # Caso esteja em um link externo separado
        ...
        col_reg_divisao = achar_coluna(df_regional, ['regional', 'divisao', 'divisão', 'gerente regional'])

        if col_reg_loja and col_reg_divisao:
            df_regional['Loja_Key'] = df_regional[col_reg_loja].astype(str).str.strip()
            df_regional['Regional_Nome'] = df_regional[col_reg_divisao].fillna('Sem Regional').astype(str).str.strip()
            
            # Cruzamento dos dados (PROCV / Merge)
...
def renderizar_dashboard():
    ...
    try:
        ...
        if perfil_usuario == "Administrador":
            ...
            # Visão de Perda por Divisão Regional
            st.subheader("🗺️ Perda por Divisão Regional (R$)")
            df_reg = (
                df_filtered[df_filtered['Valor_Limpo'] < 0]
                .groupby('Regional_Nome')['Valor_Limpo']
                .sum()
                .abs()
                .reset_index()
                .sort_values(by='Valor_Limpo', ascending=False)
            )
            df_reg['Texto_Valor'] = df_reg['Valor_Limpo'].apply(lambda x: f"-R$ {x:,.2f}")
            ...
            fig_reg.update_traces(marker_color='#ff7f0e', textposition='inside')
