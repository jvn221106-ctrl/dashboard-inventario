...
# --- LEITURA E TRATAMENTO DA PLANILHA NUVEM (DADOS + PERMISSÕES) ---
@st.cache_data(ttl=60)  # Recarrega a planilha da nuvem a cada 1 minuto
...
def load_permissoes():
    """Lê as permissões e administradores diretamente da aba PERMISSOES da planilha."""
    try:
        content = fetch_excel_bytes()
        excel_file = io.BytesIO(content)
        df_perm = pd.read_excel(excel_file, sheet_name="PERMISSOES")
        
        # Padroniza colunas
        df_perm.columns = [str(c).strip().lower() for c in df_perm.columns]
        ...
        for _, row in df_perm.iterrows():
            ...
            loja = str(row['filial']).strip().upper()
            is_adm = str(row.get('is_admin', False)).strip().upper() in ['TRUE', '1', 'SIM', 'S']
            
            if email and email != 'nan':
                emails_permitidos[email] = loja
                if is_adm:
                    admins.add(email)
                    
        return emails_permitidos, admins
    except Exception as e:
        st.error(f"Erro ao carregar aba PERMISSOES da planilha: {e}")
        # Fallback padrão caso a aba PERMISSOES ainda não exista ou dê erro
        return {
        ...
        }, {"jvn221106@gmail.com", "sergio.oliveira@vonnycosmeticos.com.br", "josue.victor@vonnycosmeticos.com.br", "controladoriaprevencao@gmail.com"}
...
def carregar_usuarios():
    emails_permitidos, _ = load_permissoes()
...
def renderizar_aba_admin():
    ...
    if st.button("🔄 Limpar Cache e Forçar Atualização das Permissões"):
        st.cache_data.clear()
        st.success("Cache limpo com sucesso! As permissões foram re-sincronizadas da planilha.")
        st.rerun()

    st.markdown("---")
    st.subheader("➕ Adicionar/Editar Permissões de Usuários")
    st.info("""
    Para adicionar ou alterar permissões de um usuário:
    1. Abra a planilha do Excel no OneDrive/SharePoint.
    2. Vá até a aba **`PERMISSOES`**.
    3. Adicione ou edite uma linha com as colunas: **`email`**, **`filial`** (ex: `B001` ou `TODAS`) e **`is_admin`** (`TRUE` ou `FALSE`).
    4. Salve o arquivo. O Dashboard sincronizará automaticamente em até 1 minuto!
    """)
...
def renderizar_dashboard():
    ...
    try:
        ...
        if email_logado in admins or loja_usuario == "TODAS":
