...
# Insira aqui o link CSV publicado do Google Sheets
URL_PLANILHA = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQUmDRQHVm5zBY9ENlbuiUTcfmA_Evcr0I9-WsEpXJY7JGkWvKNNivvJUJqlGj5Bw/pub?gid=557928447&single=true&output=csv"

@st.cache_data(ttl=60)
def load_data(url):
    # Trata valores de erro comuns do Excel/Google Sheets
    na_values = ['#ERROR!', '#N/A', '#REF!', '#VALUE!', 'nan', 'NaN', 'None', '']
    ...
    # Tratamento de textos sem permitir sujeiras do Excel
...
try:
    ...
    # Filtra mantendo apenas nomes válidos (removendo erros de planilha)
    lojas = [x for x in sorted(df['Loja_Nome'].unique()) if x.lower() not in ['nan', 'none', '', 's/ centro', '#error!']]
    lojas_sel = st.sidebar.multiselect("Selecione os Centros:", options=lojas, default=lojas)

    marcas = [x for x in sorted(df['Marca_Nome'].unique()) if x.lower() not in ['nan', 'none', '', 'sem marca', '#error!']]
