...
def load_data(uploaded_file):
    df = pd.read_excel(uploaded_file, sheet_name="VALORES INVENTÁRIOS")
...
# --- LEITURA DO ARQUIVO VIA DRAG & DROP ---
st.sidebar.title("📁 Carregar Planilha")
uploaded_file = st.sidebar.file_uploader(
    "Arraste e solte o arquivo aqui (.xlsx ou .xlsm):", 
    type=['xlsx', 'xlsm']
)
...
if uploaded_file is not None:
    try:
        ...
        # --- LÓGICA DE URL PARA BLOQUEIO POR CENTRO ---
        # Lê o parâmetro ?centro=XXXX da URL
        ...
        # Se o link tiver o parâmetro ?centro= e o centro existir no arquivo
        if centro_url and centro_url in lojas_disponiveis:
            lojas_sel = [centro_url]
            st.sidebar.info(f"🔒 **Visualização Restrita**\n\nCentro: **{centro_url}**")
        else:
            # Se for seu acesso normal (sem parâmetro de centro), exibe o multiselect completo
        ...
        if centro_url:
            st.caption(f"Exibindo dados exclusivos do centro: **{centro_url}**")
        ...
        # --- TABELA TOP 10 CENTROS COM MAIOR PERDA ---
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        # --- TABELA TOP 10 MARCAS COM MAIOR PERDA ---
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    st.info("👈 Faça o upload ou arraste a sua planilha na barra lateral para abrir o dashboard.")