...
def load_data(uploaded_file):
    ...
    for idx, row in df_preview.iterrows():
        # Converte cada item da linha para string com segurança
        row_str = [str(val).lower() for val in row.dropna().tolist()]
        texto_linha = " ".join(row_str)
        if any(term in texto_linha for term in ['centro', 'loja', 'quantidade', 'fornecedor', 'fornecedor2', 'marca', 'valor', 'qtd', 'soma de valor', 'montante', 'inventários', 'inventarios']):
            header_idx = idx
            break
    ...
    for i, col in enumerate(df.columns):
        ...
        if 'unnamed' in col_str.lower() or col_str == 'nan':
...
if uploaded_file is not None:
    try:
        ...
        # Prioriza 'fornecedor2', depois outros nomes de fornecedor/marca
        idx_marca = pegar_indice_padrao(['fornecedor2', 'fornecedor 2', 'fornecedor', 'marca', 'brand', 'inventários', 'inventarios'], 3, [idx_qtd, idx_val, idx_centro])
        ...
        col_marca = st.sidebar.selectbox("Coluna de Marca / Fornecedor (ex: Fornecedor2):", colunas_disponiveis, index=idx_marca)

        # Processamento e sanitização rigorosa de dados
        df = df_raw.copy()

        # Limpeza Numérica dos valores de Inventário
        ...
        # Despreza linhas que não representam itens/registros reais do inventário ou são cabeçalhos extras
        erros_invalidos = ['CHAMADO', 'STATUS', 'INVENTÁRIOS', 'INVENTARIOS', 'TOTAL', 'RESULTADO', 'FORNECEDOR2', 'FORNECEDOR 2']
        ...
        lojas = sorted(list(set(df['Loja_Nome'].tolist())))
        marcas = sorted(list(set(df['Marca_Nome'].tolist())))
        ...
        # KPIs calculados a partir dos dados do inventário
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.subheader("⚠️ Ranking: Top 10 Marcas/Fornecedores com Maior Perda")
        ...
        df_top10_marcas.rename(columns={'Marca_Nome': 'Marca / Fornecedor', 'Qtd_Limpa': 'Perda (Qtd)', 'Valor_Limpo': 'Perda (R$)'}, inplace=True)
        ...
        with marca_col1:
            st.subheader("📦 Perdas por Marca / Fornecedor - Todas (Qtd)")
            df_marca_qtd = (
        ...
        with marca_col2:
            st.subheader("🏷️ Perdas por Marca / Fornecedor - Todas (R$)")
            df_marca_rs = (
