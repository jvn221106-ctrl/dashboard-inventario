...
def load_data(uploaded_file):
    # Lê o arquivo Excel enviado sem recalcular fórmulas quebradas
    ...
    col_loja = achar_coluna(['centro', 'loja'])
    col_marca = achar_coluna(['fornecedor2', 'fornecedor 2', 'fornecedor', 'marca'])
    ...
    df['Loja_Nome'] = df[col_loja].fillna('Sem Centro').astype(str).str.strip()
    df['Marca_Nome'] = df[col_marca].fillna('Sem Marca').astype(str).str.strip()
    
    # Remove linhas residuais de erros de fórmula se existirem
    df = df[~df['Marca_Nome'].str.upper().isin(['#ERROR!', '#ERRO!', '#N/A', 'NAN', 'NONE'])]
...
if uploaded_file is not None:
    ...
    st.info("👈 Por favor, carregue o arquivo Excel (.xlsx ou .xlsm) no menu lateral para visualizar o Dashboard.")