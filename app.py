...
def carregar_usuarios():
    ...
    for email, loja in EMAILS_PERMITIDOS.items():
        if email not in usuarios:
            usuarios[email] = {"loja": loja, "senha": None}
            atualizou = True
        else:
...
def renderizar_tela_login():
    ...
    # MODO 1: REDEFINIÇÃO DE SENHA (FORA DO DASHBOARD)
    if st.session_state["modo_esqueci_senha"]:
        st.subheader("🔑 Redefinir Minha Senha")
        st.write("Digite seu e-mail, sua senha atual (se tiver) e defina a nova senha.")

        with st.form("form_redefinir_senha"):
            email_input = st.text_input("E-mail corporativo:").strip().lower()
            senha_atual = st.text_input("Senha Atual (deixe em branco se for 1º acesso):", type="password")
            ...
            btn_salvar = st.form_submit_button("Alterar Senha", type="primary")

        if btn_salvar:
            if not email_input or email_input not in usuarios:
                st.error("E-mail não cadastrado ou não permitido.")
                return

            dados_usr = usuarios[email_input]

            # Se já tinha senha, valida a antiga
            if dados_usr["senha"] is not None:
                if gerar_hash(senha_atual) != dados_usr["senha"]:
            ...
            usuarios[email_input]["senha"] = gerar_hash(nova_senha)
            salvar_usuarios(usuarios)
            st.success("✅ Senha alterada com sucesso! Agora você pode fazer o login.")
            st.session_state["modo_esqueci_senha"] = False
            st.rerun()
    ...
    # MODO 2: LOGIN NORMAL
        ...
        st.markdown("---")
        if st.button("❓ Esqueceu ou quer redefinir a senha?"):
...
def renderizar_dashboard():
    ...
    try:
        ...
        # Regra de Nível de Acesso (Filtro por Loja)
        ...
        # --- VISÃO EXCLUSIVA PARA ADMINISTRADORES (GRÁFICOS E RANKING DE CENTROS) ---
        if email_logado in ADMINS:
        ...
        # --- RANKING E GRÁFICOS DE MARCAS (VISÍVEL PARA TODOS) ---
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        st.error(f"Erro ao carregar os dados do arquivo Excel: {e}")
...
if not st.session_state["logado"]:
    ...
    # Barra Lateral com Informações de Usuário e Alteração de Senha
