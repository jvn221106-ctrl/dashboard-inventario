...
# ==============================================================================
# 2. CONTROLE DE AUTENTICAÇÃO E SEGURANÇA
# ==============================================================================
# 1. Exige login via Streamlit/Google
if not st.user.is_logged_in:
    st.title("🔒 Sistema de Inventários - Acesso Restrito")
    st.write("Por favor, faça login com o seu e-mail cadastrado para visualizar os dados da sua unidade.")
    st.login()
    st.stop()

# 2. Captura e limpa o e-mail autenticado
email_usuario = st.user.email.lower().strip()

# 3. Valida se o e-mail possui permissão
if email_usuario not in PERMISSOES_EMAIL:
    st.error(f"⛔ O e-mail cadastrado ({email_usuario}) não tem permissão para acessar o painel.")
    st.info("Caso necessite de acesso, entre em contato com o suporte ou administrador do sistema.")
    if st.button("Trocar de Conta"):
        st.logout()
    st.stop()

# 4. Determina qual loja o usuário autenticado pode ver
