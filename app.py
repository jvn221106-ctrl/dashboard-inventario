...
# 1. Filtro por Centro / Loja (Tratamento para evitar TypeError no sorted)
if loja_permitida == "TODAS":
    centros_unicos = [str(x) for x in df_bruto["_CENTRO_COD_"].dropna().unique() if str(x).strip() != ""]
...
# 2. Filtro exclusivo por Marca (Tratamento de segurança similar)
marcas_unicas = [str(x) for x in df_filtrado["_MARCA_"].dropna().unique() if str(x).strip() != ""]
