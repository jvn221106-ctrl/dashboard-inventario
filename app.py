...
def renderizar_tela_login():
    ...
    with st.expander("❓ Esqueceu a senha?"):
        st.info("📩 Clique no link abaixo para solicitar uma senha temporária com o Administrador:")
        
        # Formatação do texto pré-definido para URL do WhatsApp
        mensagem_pre_definida = urllib.parse.quote("Redefinição de senha do e-mail: ")
        link_whatsapp = f"https://wa.me/5511943681488?text={mensagem_pre_definida}"
        
        st.markdown(
            f"👉 **[Controladoria / Prevenção de Perdas 11 94368-1488]({link_whatsapp})**",
            unsafe_allow_html=True
        )
        st.caption("Ao fazer login com a senha temporária, o sistema exigirá a criação de uma nova senha pessoal.")
